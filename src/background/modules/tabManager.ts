...
export const handleTabCreated = () => {
  chrome.tabs.onCreated.addListener((tab) => {
    ...
    if (tab.pendingUrl === 'chrome://newtab/') {
      ...
      // 获取用户设置
      chrome.storage.local.get(['setting_isBrowserTab'], (result) => {
        const isBrowserTab = result.setting_isBrowserTab || false;
...
// 监听标签页激活事件，重新尝试填充内容
export const handleTabActivated = () => {
  chrome.tabs.onActivated.addListener(
    (activeInfo: chrome.tabs.TabActiveInfo) => {
      ...
      chrome.storage.local.get(['pendingAiSearchRequests'], (result: any) => {
        if (
          result.pendingAiSearchRequests &&
          result.pendingAiSearchRequests.length > 0
        ) {
          // 查找当前标签页的待处理请求
          const pendingRequest = result.pendingAiSearchRequests.find(
...
// 初始化所有标签页事件监听器
export const initializeTabListeners = () => {
  handleTabCreated();
  handleTabRemoved();
  handleTabActivated();
  handleTabUpdated();
